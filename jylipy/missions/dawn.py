...
from jylipy.vector import xyz2sph #Image, readfits, condition, xyz2sph, Time, num, findfile, writefits, CCDData, ImageMeasurement, CaseInsensitiveOrderedDict, findfile, ascii_read
from jylipy.geometry import subcoord
from jylipy.apext import Table, Column, units, fits
from jylipy import convenience as conv, PDS
...
class Data(object):
    ...
    alllevels = ['1a', '1b', '1c']
    allphases = ['csa', 'csr', 'cts', 'css', 'csh', 'ctl', 'csl', 'cxl', 'cxj', 'cxg', 'cto', 'cxo', 'cx2', 'cxe', 'cti', 'c2i', 'cte', 'c2e', 'vsa', 'vss', 'vth', 'vsh', 'vtl', 'vsl', 'vt2', 'vh2', 'vtc']
 ...
 v1.0.1 : JYL @PSI, 10/26/2015
   ...
   *** Important: However, this means that the units for calibrated
    ...
    def iofcal(self, image):
        ...
        photcal = self.photcal[fno-1]
        if 'flux_cal' in list(image.header.keys()):
            image.data *= (image.unit*np.pi*units.Unit('sr')/sflx).to('').value
        else:
            image.data *= (image.unit*np.pi*units.Unit('sr')/(texp*photcal*sflx)).to('').value
        image.unit = ''
...
def path(ids, level='1a', base=DAWN_DIR+'data/fc2'):
    '''Returns the full path of input filenames or IDs

    ids : file name(s) in string or list of array, or exposure IDs
          in integers
    '''
    ...
    if isinstance(ids[0], (str, bytes)):
        ids = list(map(int,[basename(x).split('_')[0][5:] for x in ids]))

    filelist = ascii_read(DAWN_DIR+'data/fc2/meta/level1a_IMG.csv')
    files = []
    for i in ids:
        directory = filelist.query('eid',i)['dir'][0]
        imgfile = findfile(base+'/level-'+level+'/'+directory,str(i)+'_')
        if len(imgfile)==1:
            files.append(imgfile[0])
        else:
            print(f'Problem finding file of Exposure ID {i}, skipped.')
    return files
 ...
 **kwargs :
 ...
 v1.0.1, 1/11/2016, JYL @PSI
   Added keyward `spice`
    ...
    from jylipy.pysis_ext import dawnfc2isis, fits2isis, flip, copylabel, spiceinit
    ...
    print('Generating cube file from raw data: {0}'.format(basename(rawfile)))
    ...
    print('Processing input file: {0}'.format(basename(infile)))
    ...
    # Copy labels
    print('Copying labels: {0}'.format(basename(outfile)))
    ...
    # spiceinit
    if spice:
        print('spiceinit: {0}'.format(basename(outfile)))
        args = {'from': outfile}
        args.update(kwargs)
        args.update(log)
        spiceinit(**args)
...
def extract_phodata(*args, **kwargs):
    out = Photometry.PhotometricData()
    out.extract(*args, **kwargs)
    return out


# Old code
def extract_phodata_old(illfile, ioffile=None, backplanes=['Phase Angle', 'Local Emission Angle', 'Local Incidence Angle', 'Latitude', 'Longitude'], outfile=None, bin=1, overwrite=False):
...
def collect_fc_phodata(*args, **kwargs):
    out = Photometry.PhotometricData()
    out.populate(*args, **kwargs)
    return out


# Old code
def collect_fc_phodata_1(illfile, outfile, ioffile=None, backplanes=['Phase Angle', 'Local Emission Angle', 'Local Incidence Angle', 'Latitude', 'Longitude'], bin=1, overwrite=False):
 ...
 10/26/2015, JYL @PSI
    ...
    if forfile:
        ...
        if (len(outsegs) > 1) and ((outsegs[-1].lower() != 'fits') or (outsegs[-1].lower() != 'fit')):
            outsegs[-1] = 'fits'
            out = '.'.join(outsegs)
        writefits(out, result.astype('ubyte'), overwrite=overwrite)
    else:
        out = result
    return out
